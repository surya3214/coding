// AC - F yeaaaaaaaaaaaahhhhhhhhhhhh!
#include <bits/stdc++.h>
#define BUFFER 5
...
struct bit_set { // structure represents the numbers of range [1 - 10^5] present in a range
  ...
  for (blk = 1; blk < block_count; blk++) { // combine blockRange[blk][i - 1] with block[i] to make blockRange[blk][i]
    ...
    for (int next_blk = blk + 1; next_blk <= block_count; next_blk++)
      for (int set_id = 1; set_id <= SETS_MAX; set_id++)
        blocksRange[blk][next_blk].t.sets[set_id] = blocksRange[blk][next_blk - 1].t.sets[set_id] | blocks[next_blk].t.sets[set_id];
  }
}
int my_mod(int x) {
  while (x >= n)
...
int findKth(int l, int r) {
  ...
  cnt = 0;
  for (int set_id = 1; set_id <= SETS_MAX; set_id++) {
    to_be_found = __builtin_popcountll(b.t.sets[set_id]);
    if (cnt + to_be_found < k) cnt += to_be_found;
    else { // kth number is here
      while (b.t.sets[set_id]) {
        pos = __builtin_ctzll(b.t.sets[set_id]);
        b.t.sets[set_id] ^= (1LL << pos);
        if (++cnt == k)
